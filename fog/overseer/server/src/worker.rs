    ...
    OverseerError: From<DB::Error>,
{
    ...
    /// FogIngestGrpcClient that communicates with a node that isn't stopped.
    ...
    ///      These are ingress keys that Fog Ingest needs to be using to scan
    ///      blocks (but the issue is that Fog Ingest isn't currently doing that
    ///      because all nodes are idle).
    ///   2. If there are more than one outstanding key, disarm and send
    ///      an alert (todo).
    ///   2. If there's one oustanding key, and try to find an idle node that
    ///      contains that key.
    ///         a. If you find one, great! Just activate that node. If
    ///            activation is unsuccessful, then return an error and
    ///            return to the overseer polling logic.
    ///         b. If you don't find an idle node with that key, then you have
    ///            to report that key as lost.
    ///  3. If none of the nodes contained an outstanding key, then no node
    ///     will be activated. We now have to:
    ...
    fn perform_automatic_failover(
        ...
        match inactive_outstanding_keys.len() {
            ...
            1 => log::info!(self.logger, "Found 1 outstanding keys."),
            _ => {
                log::error!(self.logger, "Found multiple outstanding keys. This requires manual intervention. Disarming.");
                self.stop_requested.store(true, Ordering::SeqCst);
                return Err(OverseerError::MultipleOutstandingKeys("Multiple outstanding keys found. This is unexpected and requires manual intervention. As such, we're disarming overseer. Take action and then rearm.".to_string()));
            }
        }

        let mut was_one_node_activated = false;
        if let Some(inactive_outstanding_key) = inactive_outstanding_keys.get(0) {
            was_one_node_activated = self.try_to_activate_node_for_inactive_outstanding_key(
                &inactive_outstanding_key,
                &ingest_summary_node_mappings,
            )?;
        }

        if was_one_node_activated {
            log::info!(
                self.logger,
    ...
    fn get_inactive_outstanding_keys(
        ...
        // An outanding key is one that is still obligated to be scanning blocks
        // on behalf of users.
        let outstanding_keys_filters = IngressPublicKeyRecordFilters {
            ...
            // are still supposed to be scanning.
        ...
        //
        // TODO: Add a config that allows us to set this start block at least.
    ...
    ///
    /// returns bool - true if a node was successfully activated.
    fn try_to_activate_node_for_inactive_outstanding_key(
        ...
        for ingest_summary_node_mapping in ingest_summary_node_mappings {
            let node_ingress_key = match CompressedRistrettoPublic::try_from(
                ...
                Ok(key) => key,
                // TODO: Should we panic here? This really shouldn't happen.
                Err(_) => continue,
            };
            if inactive_outstanding_key.eq(&node_ingress_key) {
                // We have to keep trying to activate the node until it
                // becomes active.
                //
    ...
    fn report_lost_ingress_key(
        ...
        let result = retry_with_index(Fixed::from_millis(200), |current_try| {
            if current_try as u8 > Self::NUMBER_OF_RETRIES {
                ...
                .report_lost_ingress_key(inactive_outstanding_key)
            {
                ...
                Err(_) => {
                    let number_of_remaining_tries = Self::NUMBER_OF_RETRIES - current_try as u8;
